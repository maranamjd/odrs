...
function getDashboardData(){
  $.ajax({
    ...
    success: function(data){
      ...
      let h1s = ['employee', 'branch', 'office', 'course', 'document', 'doctype', 'subject', 'survey'];
      for (var x in h1s) {
        let a = $('#'+h1s[x]+"_active").find('a');
        $('#'+h1s[x]+"_active").text(data[h1s[x]]['active']);
        $('#'+h1s[x]+"_active").append(a);
        a = $('#'+h1s[x]+"_trash").find('a');
        $('#'+h1s[x]+"_trash").text(data[h1s[x]]['trash']);
        $('#'+h1s[x]+"_trash").append(a);
      }
